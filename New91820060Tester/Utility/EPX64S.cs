...
namespace New91820060Tester
{
    ...
    public class EPX64S
    {
        ...
        //静的メンバー
        private static int Number;              // Devices Number
        public static int[] SerialNumList;
        ...
        public static bool GetSerialNumList()
        {
            try
            {
                // Device Number
                EPX64S_GetNumberOfDevices(ref Number);
                if (Number == 0) return false;


                var ListHandle = new List<int>();
                foreach (var i in Enumerable.Range(0, Number))
                {
                    int serial = 0;
                    EPX64S_GetSerialNumber(i, ref serial);
                    ListHandle.Add(serial);
                }

                SerialNumList = ListHandle.ToArray();
                return true;
            }
            catch
            {
        ...
        //EPX64Sを複数使用する場合（事前にシリアルラベルに表記されているSNを調べておくこと）
        public bool InitEpx64S(byte DirectionData, int serial)
        {
            this.Status = EPX64S_OpenBySerialNumber(serial, ref this.hDevice);
        ...
        public bool Close()
        {
            try
            {
                if (this.hDevice != System.IntPtr.Zero)
                {
                    ////すべてのポートをLに設定する（設定が入力でも大丈夫）
                    //this.OutByte(PORT.P0, 0x00);
                    //this.OutByte(PORT.P1, 0x00);
                    //this.OutByte(PORT.P2, 0x00);
                    //this.OutByte(PORT.P3, 0x00);
                    //this.OutByte(PORT.P4, 0x00);
                    //this.OutByte(PORT.P5, 0x00);
                    //this.OutByte(PORT.P6, 0x00);
                    //this.OutByte(PORT.P7, 0x00);
